 ...
 */
export function createLayout(settings: SettingsState) {
  if (!worker) {
  ...
  for (const record of queryResult.records) {
    ...
    nodes.push(
      `${record.id} [id="${record.id}" label="${displayText}"
            ...
            fillcolor="${
              recordColor ? colorUtils.getHexForColor(recordColor) : 'white'
            }"]`
    );

    const linkedRecordCellValues =
      (record.getCellValue(field.id) as Record[]) || [];
